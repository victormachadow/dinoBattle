...
local options = { numFriends = 0 , numEn = 1 , dificult = 10 ,  side = "esq" , arrows = false , en1 = "green" , en2 = "green" , en3 = "green" }
  ...
  resButton = widget.newButton
{
 ...
 resButton.x = centerX
  ...
  transition.to(resButton,{ y = centerY-50 , time=800, alpha=1})
...
if ( player.y < event.y and player.x == event.x )then--   en desce apenas
...
bkg:removeEventListener("touch" , tapa2 )
...
bkg:addEventListener("touch" , tapa2 )
--print(num)
...
transition.moveTo( en1, { x = math.random(_W/2 + 50 , _W -50 ), y=math.random(_H/2 + 50 , _H -50 ), time=math.random(1000,2000) , onComplete=atack() } )
 ...
 function myFunction (en)

    speed = 300
    deltaX = en.x - player.x
    deltaY = en.y - player.y
...
normDeltaY = deltaY / math.sqrt(math.pow(deltaX,2) + math.pow(deltaY,2))
    ...
    if ( en.x > player.x  ) then  bulletEn.x = en.x-40        end
    if ( en.x < player.x  ) then  bulletEn.x = en.x+40        end
    --bulletEn.x = en.x
    --bulletEn.y = en.y-80
    if ( en.y > player.y  ) then  bulletEn.y = en.y-40        end
    if ( en.y < player.y  ) then  bulletEn.y = en.y+40        end
    ...
    --bulletEn:setLinearVelocity( math.cos( angle ) *-1 * speed, math.sin( angle ) * speed )
    --bulletEn:applyLinearImpulse( math.cos( angle )/80 * speed , math.sin( angle )/80 * speed , en.x , en.y )
    bulletEn:applyLinearImpulse(((normDeltaX * 30)/100) * -1  , ((normDeltaY * 30 )/100 ) * -1 , bulletEn.x , bulletEn.y )
...
transition.moveTo( en , { x = math.random( 0, _W -50 ), y=math.random( 0 , _H -50 ), time=math.random(1000,2000) , onComplete = function(en)
...
transition.moveTo( en2 , { x= math.random(_W/2 + 50 , _W -50 ), y=math.random(_H/2 + 50 , _H -50 ), time=math.random(1000,2000) , onComplete=atack22() } )
...
transition.moveTo( en3 , { x = math.random(_W/2 + 50 , _W -50 ), y=math.random(_H/2 + 50 , _H -50 ), time=math.random(1000,2000) , onComplete=atack3 } )
...
timers[1] = timer.performWithDelay( 1500 , ataca1 , -1 )
--timers[2] = timer.performWithDelay( 1500 , ataca2 , -1 )
--timers[3] = timer.performWithDelay( 1500 , ataca3 , -1 )
...
bkg = display.newImage( "bg2.jpg", centerX, centerY )   

sceneGroup:insert(bkg)


 borderBottom = display.newRect( 0, _H , _W*2, 20 )
